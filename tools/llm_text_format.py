"""
Plain-text cleanup for LLM output — Python port of Pocket's Kotlin
ResponseSanitizer + MessageTextFormatter.readableAssistantText.
...
Kept in sync manually with:
  pocket-api/.../ResponseSanitizer.kt
  feature-chatbot/.../MessageTextFormatter.kt
"""
...
def sanitize(inp: str) -> str:
    """Mirror ResponseSanitizer.sanitize — drop reasoning blocks and chat control tokens."""
...
# --- StreamingResponseSanitizer parity (subset of tokens, length-sorted) ---
...
def _main(argv: list[str]) -> int:
    """CLI: stdin → readable_assistant_text, or --sanitize-only for ResponseSanitizer only."""
